...
void EverQuestMod::LoadSpellData()
{
    ...
    QueryResult queryResult = WorldDatabase.Query("SELECT SpellID, AuraDurationBaseInMS, AuraDurationAddPerLevelInMS, AuraDurationMaxInMS, AuraDurationCalcMinLevel, AuraDurationCalcMaxLevel, RecourseSpellID, SpellIDCastOnMeleeAttacker, FocusCategory FROM mod_everquest_spell ORDER BY SpellID;");
    if (queryResult)
    {
        do
        {
            ...
            everQuestSpell.SpellFocusCategory = fields[8].Get<uint32>();
