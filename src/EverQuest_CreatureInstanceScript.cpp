...
public:
    ...
    struct EverQuest_CreatureInstanceScriptAI : public ScriptedAI
    {
        ...
        {
            return EverQuest->GetTerrainSnappedZ(me, priorX, priorY, priorZ, initialTargetX, initialTargetY, initialTargetZ,
                foundValidZ, CreatureInstanceData.DisableGroundContour, minZ, maxZ);
        }
        ...
        bool BuildPathAndStartPointMovementToTarget(float initialTargetX, float initialTargetY, float initialTargetZ, uint32 moveType, bool run = false)
        {
            ...
            float terrainSnappedTargetZ = initialTargetZ;
            if (EverQuest->BuildTerrainSnappedMovementPath(me, initialTargetX, initialTargetY, initialTargetZ, CreatureInstanceData.DisableGroundContour,
                CreatureInstanceData.RoamMinZ, CreatureInstanceData.RoamMaxZ, MovementType == EQ_CREATURE_MOVEMENT_CUSTOM_ROAMING,
                waypointPath, terrainSnappedTargetZ) == false)
                return false;
