...
void EverQuestMod::RefreshMonkArmorAuraForPlayer(Player* player)
{
    ...
    // Mail, plate or a shield anywhere makes it heavy, and anything else (cloth, leather, nothing) is light
    ...
    if (PlayerHasClassAura(player, EQ_CLASSAURA_SPELL_MONK_AURA) == true)
    {
        ...
        for (uint8 equipSlotIndex = EQUIPMENT_SLOT_START; equipSlotIndex < EQUIPMENT_SLOT_END; ++equipSlotIndex)
        {
            ...
            if (itemTemplate == nullptr || itemTemplate->Class != ITEM_CLASS_ARMOR)
                continue;
            if (itemTemplate->SubClass == ITEM_SUBCLASS_ARMOR_MAIL || itemTemplate->SubClass == ITEM_SUBCLASS_ARMOR_PLATE || itemTemplate->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD)
            {
                isWearingHeavy = true;
                break;
            }
...
{
    if (player == nullptr || spell == nullptr || strict == false)
        return;
    ...
    if (baseCastTimeInMS > 0 && isDirectHeal == true && isSingleTarget == true && target == player && ConfigSystemClassAuraMonkSelfHealCastTimeReductionPercent > 0
        && ConfigSystemClassAuraMonkSelfHealCastTimeReductionPercent < 100 && PlayerHasClassAura(player, EQ_CLASSAURA_SPELL_MONK_AURA) == true)
        castTimeMultiplier *= (100.0f - (float)ConfigSystemClassAuraMonkSelfHealCastTimeReductionPercent) / 100.0f;
    ...
    if (cadenceSpellID != 0 && isDirectHeal == true && isSingleTarget == true && target != nullptr && isCompleteHeal == false
    ...
    if (edgeSpellID != 0 && baseCastTimeInMS > 0 && spellInfo->IsPositive() == false && spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC
...
void EverQuestMod::HandleClassAuraSpellCast(Player* player, Spell* spell)
{
    ...
    // Cleric
    if (spellInfo->HasEffect(SPELL_EFFECT_HEAL) == true && PlayerHasClassAura(player, EQ_CLASSAURA_SPELL_CLERIC_AURA) == true)
    {
        if (spellInfo->IsAffectingArea() == true)
        {
            uint32 cadenceSpellID = GetClassAuraSpellID(EQ_CLASSAURA_SPELL_CLERIC_CADENCE);
            if (cadenceSpellID != 0)
